class EventController < ApplicationController
        ...
        if artists
          artists.each do |artist|
            if artist['name']
              ...
              if check_artist_event gl_artist, @event.date
                gl_artist.next_event_at = @event.date
                gl_artist.save!
              end
  ...
  def check_artist_event(artist, date)
    if artist.next_event_at == nil
      return true
    end

    if artist.next_event_at < Time.now
      return true
    end

    if artist.next_event_at > date
      return true
    end
